...
tf.app.flags.DEFINE_string('log_save_path', './logs/test_log', 'Directory where to save tensorboard log')
tf.app.flags.DEFINE_string('model_save_path', './model/test_model', 'Directory where to save model weights')
#tf.app.flags.DEFINE_string('ckpt_save_path', './checkpt', 'Directory where to save checkpoint')
tf.app.flags.DEFINE_integer('batch_size', 64, 'batch size')
tf.app.flags.DEFINE_integer('iteration', 43, 'iteration')
tf.app.flags.DEFINE_float('weight_decay', 0.0001, 'weight decay')
tf.app.flags.DEFINE_float('dropout', 0.5, 'dropout')
tf.app.flags.DEFINE_float('epochs', 82, 'epochs')
...
def main(_):
    train_x, train_y, test_x, test_y = fetch_data()
    train_x, test_x = color_preProcess(train_x, test_x)
    ...
    def run_testing(sess):
        ...
        for it in range(10):
            ...
            loss_, acc_ = sess.run([cross_entropy, accuracy],
                                   feed_dict={x: batch_x, y_: batch_y, use_bn: False, keep_prob: 1.0})
            loss += loss_ / 10.0
            acc += acc_ / 10.0
